...
public class OrderInfoController {
	...
	@GetMapping("")
	@ApiOperation(value = "사용자의 주문내역 list 조회", notes = "year 없으면 6개월, 파라메터값 넣으면 해당 년도 주문내역 list "
		                                                   + "반환", response = OrderInfoListGetResponse.class)
	@ApiResponses(value = {
	...
	public ResponseEntity<OrderInfoListGetResponse> getOrderInfoList(
		@ApiIgnore @RequestHeader("Authorization") String token,
		@ApiParam(value = "조회 년도,없으면 6개월", example = "2021") Integer year) {
		try {
	...
	@ApiOperation(value = "사용자의 주문내역 조회", notes = "해당 주문 ID의 주문 내역 조회", response =
		                                                                    OrderInfoListGetResponse.class)
	@ApiResponses(value = {
		@ApiResponse(code = 200, message = "OK"),
		@ApiResponse(code = 400, message = "Bad Request"),
		@ApiResponse(code = 401, message = "Unauthorized"),
	...
	public ResponseEntity<OrderInfoGetResponse> getOrderInfo(
		@ApiIgnore @RequestHeader("Authorization") String token,
		@PathVariable @ApiParam(value = "주문내역 ID") String orderInfoId) {
	...
	@GetMapping("/market")
	@ApiOperation(value = "픽업 매니저의 주문내역 조회", notes = "해당 픽업매니저의 시장 주문 내역 조회", response =
		                                                                          OrderInfoListGetResponse.class)
	@ApiResponses(value = {
		@ApiResponse(code = 200, message = "OK"),
		@ApiResponse(code = 400, message = "Bad Request"),
	...
	public ResponseEntity<OrderInfoListGetResponse> getMarketOrderInfo(
		@ApiIgnore @RequestHeader("Authorization") String token) {
		...
		try {
			...
			if (user.getUserType().getId() == 3L) {
				...
				return ResponseEntity.ok(OrderInfoListGetResponse.of(200, "픽업 리스트",
					orderInfoList));
			} else {
				return ResponseEntity.status(400)
					       .body(OrderInfoListGetResponse.of(400, "픽업 매니저가 아닙니다!", null));
			}
		} catch (AuthException ex) {
			return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
				       .body(OrderInfoListGetResponse.of(401, ex.getMessage(), null));
	...
	public ResponseEntity<BaseResponseBody> checkStatus(
		@ApiIgnore @RequestHeader("Authorization") String token,
		@RequestBody OrderInfoPatchRequest infoPatchRequest) {
		if (infoPatchRequest.getOrderStatusId() >= 4L
			    || infoPatchRequest.getOrderStatusId() <= 0L) {
			return ResponseEntity.status(HttpStatus.BAD_REQUEST)
				       .body(BaseResponseBody.of(400, "주문 상태 ID 확인"));
		}
		if (infoPatchRequest.getOrderInfoId() == null
			    || infoPatchRequest.getOrderStatusId() == null) {
			return ResponseEntity.status(HttpStatus.BAD_REQUEST)
